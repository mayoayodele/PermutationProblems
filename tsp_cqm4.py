     ...
     #'dantzig42', 'berlin52', 'brazil58',  'st70',
       'rd100',   'kroA100', 'kroB100', 'pr107',  'gr120'
        ...
        for j in range(reduced_size):
            cqm.add_constraint((sum([model[j][i] for i in range(reduced_size)]) -1)**2  == 0, label = 'constraint_row'+ str(j))
            cqm.add_constraint((sum([model[i][j] for i in range(reduced_size)]) -1)**2  == 0, label = 'constraint_column' + str(j))
