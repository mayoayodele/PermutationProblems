...
def main():
    ...
    for problem_name in [  #'had12', 'had14', 'had16', 'had18', 'had20',  'rou12', 'rou15', 'rou20', 
        ...
        for j in range(problem_size):
            cqm.add_constraint((sum([model[j][i] for i in range(problem_size)]) -1)**2  == 0, label = 'constraint_row'+ str(j))
            cqm.add_constraint((sum([model[i][j] for i in range(problem_size)]) -1)**2  == 0, label = 'constraint_column' + str(j))
